...
> **Note:** Deployment is handled by the `deploy-pages` job in `.github/workflows/ci.yml`. It runs automatically on every push to `master` after the WASM build succeeds, pushing to the `gh-pages` branch.
